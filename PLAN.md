...
- 单元测试：配置 schema、权限判定、触发器匹配、日志分片、时间线关联、secret redaction、Stronghold 主密码换密成功/失败回退、旧密码失效与 `secretRef` 保持。
