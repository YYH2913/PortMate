...
describe("terminal timestamp state", () => {
  it("normalizes timestamps, keeps the first event per line, and bounds old rows", () => {
    expect(normalizeTerminalTimestamps([
      ...
      { line: 1, ts: "2026-08-09T01:02:01.000Z" },
      { line: 3, ts: "2026-08-09T01:02:03.000Z" },
    ]);
  });

  it("maps only visible unique lines to viewport rows", () => {
    expect(visibleTerminalTimestamps([
      ...
      { line: 10, row: 0, ts: "2026-08-09T01:02:02.000Z" },
      { line: 12, row: 2, ts: "2026-08-09T01:02:04.000Z" },
    ]);
  });
});
