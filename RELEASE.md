...
- [ ] On macOS, `npm run test:macos-package` passes against the freshly built app and DMG, including Info.plist metadata, portable symlinks, release-source SHA-256 checks for unsigned binaries or strict code-signature verification for signed binaries, direct app/DMG payload equality, DMG verification, a read-only mount audit, and two-launch Store/IPC credential verification.
