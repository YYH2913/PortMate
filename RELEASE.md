...
- [ ] On Linux, `npm run test:linux-package` passes on X11/Xvfb against the freshly built DEB, RPM, and AppImage from a space-containing extraction/runtime root, including full file/symlink permission checks plus three successful restart/legacy-migration launches and one fail-closed two-Store conflict start from every extracted package.
