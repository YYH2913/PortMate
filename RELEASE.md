...
- [ ] `npm run test:mcp-stdio-client`, `npm run test:mcp-http-client`, `npm run test:mcp-python-client`, `npm run test:mcp-go-client`, `npm run test:mcp-rust-client`, and `npm run test:mcp-ruby-client` pass.
...
- [ ] Run every official TypeScript/Python/Go/Rust/Ruby MCP SDK check with `PORTMATE_MCP_BINARY` set to the bridge extracted from each package.
