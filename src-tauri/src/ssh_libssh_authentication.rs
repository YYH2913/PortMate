...
pub(super) use gssapi::validate_libssh_gssapi_credential_cache;
    ...
    for method in auth_order {
        let status = match method {
            AuthMethod::GssapiWithMic => {
                ...
                if let Err(error) = validate_libssh_gssapi_credential_cache(
                    std::env::var_os("KRB5CCNAME").as_deref(),
                ) {
                    failures.push(error);
                    continue;
                }
                match session.userauth_gssapi() {
                    Ok(status) => status,
                    Err(error) => {
                        failures.push(format!("GSSAPI authentication failed: {error}"));
                        methods = session.userauth_list(None).map_err(|refresh_error| {
                            format!(
                                "libssh auth method refresh failed after GSSAPI error: {refresh_error}"
                            )
                        })?;
                        continue;
                    }
