...
- SCP：上传、下载、远端 `cp` 复制。下载的文件头、正文和完成确认，以及受 flow control 的上传/下载写入，均保持同一个 russh I/O future 并每 100 ms 检查取消；连续 60 秒没有数据或写入完成会以空闲超时失败。上传和下载的正常、取消、超时、协议错误与非零状态路径都使用有界 channel 收尾；EOF 后保留 1 秒等待迟到的 `exit-status`。下载只在 SCP 完成确认和远端成功状态均通过后才把 `.portmate-part` 提升为最终文件；标准 `scp -f` 不支持偏移协商，因此每次下载从字节 0 重写 part，不会把未验证的旧前缀与新后缀拼接。下载协议只从 stdout 解析控制流和文件数据，stderr 独立限制为 64 KiB；文件头和错误行必须在 64 KiB 内完整换行，防止登录提示污染文件或异常对端无界增长协议缓冲。
