...
1. `lib.rs` 已累计减少约 28,000 行，并按以下边界持续拆分；主文件不再直接拥有 Tauri command：
   ...
   - transfer：`transfer_commands.rs`、`transfer_runtime.rs`、`file_commands.rs`、`file_operations.rs`、`file_batch.rs`、`file_transfer.rs`、`scp_protocol.rs` 和 `modem_transfer.rs` 承接传输完整桌面命令适配、文件桌面命令适配、共享本地/SFTP 列表/属性/路径安全基础、队列、批次规划、Local/SFTP/SCP/remote-copy 与 X/Y/ZModem 数据面。
