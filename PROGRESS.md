...
1. `lib.rs` 已累计减少约 22,600 行，并按以下边界持续拆分：
   ...
   - transfer：`transfer_runtime.rs`、`file_commands.rs`、`file_batch.rs`、`file_transfer.rs`、`scp_protocol.rs` 和 `modem_transfer.rs` 承接文件桌面命令适配、队列、批次规划、Local/SFTP/SCP/remote-copy 与 X/Y/ZModem 数据面。
