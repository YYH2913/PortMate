...
1. `tmux_protocol.rs`、`telnet_protocol.rs`、`proxy_protocol.rs` 已承接对应协议状态机；`serial_capture.rs` 迁出有界 Serial capture 模型、增量快照和 session registry；`ssh_security.rs` 进一步迁出 ssh-agent signer/auth、一次性信任、TOFU/Host Key 持久化、`lastSeen` 和 Profile 镜像同步，`ssh_health.rs` 迁出三层健康命令与报告；`one_key_prompt.rs` 迁出终端转义清洗、凭据提示识别、事件时效和字段匹配验证；`state_snapshot.rs` 迁出 Store/Portable Stronghold 文件锁、SHA-256 版本、跨实例陈旧写入防护和 Stronghold 提交/换密 guard；`sqlite_schema.rs` 迁出 SQLite 表、索引、schema version 和兼容列迁移；`sqlite_mirror.rs` 迁出 Profile/runtime 全量镜像和 event/audit/timeline/Sysmon 增量协调；`sqlite_store.rs` 迁出 SQLite load、FULL-sync transaction、migration checkpoint、revision 和提交后读回验证。`lib.rs` 已累计减少约 2,000 行，但 TCP/Telnet 生命周期、transport、transfer、MCP、storage/Stronghold orchestration 仍需继续按所有权边界机械拆分。
