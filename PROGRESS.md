...
1. `lib.rs` 已累计减少约 23,300 行，并按以下边界持续拆分：
   ...
   - security/profile：`ssh_security.rs`、`ssh_host_key_commands.rs`、`ssh_identity_commands.rs`、`ssh_host_key_scan.rs`、`ssh_health.rs`、`one_key_prompt.rs`、`profile_commands.rs` 和 `profile_normalization.rs` 承接 SSH 信任、完整 Host Key 命令/Store mirror/并发字段合并、identity/agent 枚举/更新/轮换/删除、直连/多跳扫描、健康检查、提示识别，以及 Profile 保存事务/归一化。
