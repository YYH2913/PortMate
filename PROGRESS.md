...
1. `lib.rs` 已累计减少约 29,000 行，并按以下边界持续拆分；主文件不再直接拥有 Tauri command：
   ...
   - security/profile：`ssh_security.rs`、`ssh_host_key_commands.rs`、`ssh_identity_commands.rs`、`ssh_host_key_scan.rs`、`ssh_health.rs`、`one_key_commands.rs`、`one_key_prompt.rs`、`profile_commands.rs`、`profile_normalization.rs` 和 `profile_security.rs` 承接 SSH 信任、完整 Host Key 命令/Store mirror/并发字段合并、identity/agent 枚举/更新/轮换/删除、SSH/Tmux/代理 Profile 访问、Client Identity 合并与 Secret 引用规范化/全局计数、直连/多跳扫描、健康检查、OneKey CRUD/发送/提示复核，以及 Profile 保存事务/归一化。
