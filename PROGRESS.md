...
1. `lib.rs` 已累计减少约 29,000 行，并按以下边界持续拆分；主文件不再直接拥有 Tauri command：
   ...
   - session I/O/logging：`session_commands.rs`、`outbound_io.rs`、`outbound_events.rs`、`session_events.rs`、`sysmon_commands.rs`、`sysmon_runtime.rs`、`terminal_export_commands.rs`、`trigger_runtime.rs`、`log_commands.rs` 和 `log_storage.rs` 承接 session 查询/输入/resize/open/delete/close 命令适配、每会话出站 lane、跨协议 writer、双向事件、Sysmon 桌面命令以及本机/远端采集、四平台解析和网络地址回退、终端导出桌面命令适配、触发动作/受限子进程/发送批次编排、日志/归档桌面命令、append-only shard 存储与管理，以及 bytesRef v1/v2 解析、范围读取和 SHA-256 替换检测。
   ...
   命令适配 ownership、Tmux control runtime 与 Sysmon 采集/解析边界已收敛；下一步继续拆分仍在主文件中的 system event sink 编排和大型测试夹具。
