...
| 隧道 | 大部分实现 | local/remote/dynamic、运行中列表、停止入口、连接数/字节/最后错误、SOCKS5 5 秒握手 deadline、三模式 10 秒连接 setup 边界、全应用 256 活动客户端上限、channel-open 超时断线清理、remote reverse 目标失败/饱和/无路由/非法端口的 1 秒 channel 收尾、监听器终止、Linux/FreeBSD/macOS remote forward 被动探测、撤销后重建、cancel 失败本地收敛、SSH 断线清理和重连后原规格恢复已接入；OpenSSH 三模式、撤销/恢复/停止、重建失败隔离和 SOCKS5 错误协议已覆盖，真实 BSD/macOS 主机和更广服务端矩阵待补。 |
