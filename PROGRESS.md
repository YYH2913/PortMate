...
- WindTerm 级别的可配置 keymap、pane 移出独立窗口和跨 group 移动仍不完整；默认分屏创建、方向焦点移动、关闭、交换、zoom、比例调整和恢复已可用。
...
当前 Rust workspace 自动化测试总数为 217：`portmate` 156、`portmate-kdf` 1、`portmate-core` 33、`portmate-mcp` 27；`npm test` 另有 82 个前端 transfer/selection/presentation/log-shard/workspace/workspace-hotkey/trigger/sync-input/secret-migration/SSH-health/TCP-health/Serial-health/Serial-capture/proxy/Sysmon-history 单元测试。
...
| WindTerm 风格工作台 | 大部分实现 | 主布局和菜单、最多 16 pane/8 层的递归水平/垂直分屏、可调且持久化的比例、v1 snapshot/旧 key 迁移、pane/active/tab color 恢复、WindTerm 默认分屏/方向焦点/关闭/zoom 快捷键、方向 pane 交换、启动会话策略及 xterm/CSS lazy chunk 已有；可配置 keymap、pane 独立窗口和跨 group 移动仍待补。 |
...
2. pane session binding、启动自动连接、标签颜色、workspace restore、v1 自动迁移、任意递归嵌套分屏，以及 WindTerm 默认分屏创建/方向焦点/关闭/zoom 快捷键和方向 pane 交换已完成；继续补可配置 keymap、pane 独立窗口和跨 group 移动。
