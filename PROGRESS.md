...
1. `lib.rs` 已累计减少约 17,200 行，并按以下边界持续拆分：
   - transport/runtime：`tmux_protocol.rs`、`telnet_protocol.rs`、`proxy_protocol.rs`、`tcp_transport.rs`、`serial_transport.rs`、`shell_transport.rs` 和 `serial_capture.rs` 承接协议状态机、TCP/Telnet/Serial runtime、Shell PTY/子进程 runtime、端口打开与读循环、最新 Profile 自动重连、串口线制映射与 capture。
