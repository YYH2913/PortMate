...
- Runtime summary 已记录 `lastDisconnect`/`lastDisconnectReason`，SQLite mirror 同步保存，桌面会话工具栏会显示最近断开时间和原因；SSH/TCP/Telnet/Serial 自动重连已有初版，SSH 有协议 keepalive，TCP/Telnet 有 Profile 级可配置 OS keepalive/重连延迟，断线后会进入 `Reconnecting` 并后台重试；SSH/Serial 健康阈值、更广故障矩阵和更深连接健康诊断还不完整。
...
5. 扩展自动重连、断线恢复和连接健康检测：SSH、TCP/Telnet 与 Serial 会在重试前加载最新 Profile，并在尝试完成时拒绝已过期配置；TCP/Telnet/Serial 在 pending/connected 阶段关闭重连均能立即或下次断线时收敛，TCP/Telnet 的重连延迟与 OS keepalive 已可按 Profile 配置，SSH 协议 keepalive、runtime 最近断开时间/原因已可见；下一步补 SSH/Serial 健康阈值、更广故障矩阵和更深健康诊断。
