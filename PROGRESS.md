...
| Tmux | 大部分实现 | list/attach/new-session、按 window 聚合的 pane inspection、经 SSH exec 确认的 `synchronize-panes` 开关、session/window 新建/重命名/确认关闭、pane activate/split/swap/resize/break/move/确认关闭、五种内置布局，以及同一 SSH runtime 上多个 tmux session 的独立 control-mode 监听/刷新/停止/关闭清理可用；仍缺真实多版本 tmux server/control 协议矩阵。 |
...
| 隧道 | 大部分实现 | local/remote/dynamic、运行中列表、停止入口、连接数/字节/最后错误、SOCKS5 5 秒握手 deadline、三模式 10 秒连接 setup 边界、channel-open 超时断线清理、监听器终止、Linux/FreeBSD/macOS remote forward 被动探测、撤销后重建、cancel 失败本地收敛、SSH 断线清理和重连后原规格恢复已接入；OpenSSH 三模式、撤销/恢复/停止、重建失败隔离和 SOCKS5 错误协议已覆盖，真实 BSD/macOS 主机和更广服务端矩阵待补。 |
