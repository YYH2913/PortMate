...
- 触发器支持多个 contains/regex 规则和每条规则的有序多动作编辑；动作包括高亮、通知、时间线标记、本地命令、发送文本、自定义链接和 bell/chime/alert 声音。每个 Profile 最多 64 条规则、每条 16 个动作，ID/名称/matcher/正则编译体积/动作载荷/颜色/声音均由后端验证，紧凑编辑器同步限制输入和原位禁用添加按钮；保存拒绝重复、超限和非法定义，旧 Store 加载与运行时求值 fail-closed，不执行被截断的命令。运行时视觉/声音效果通过 Tauri event 立即送达桌面，本地命令与发送文本保留后端 dispatch，并记录 system event/timeline 诊断。本地命令按应用限制为最多 4 路并发、30 秒总时限和 stdout/stderr 各 64 KiB；饱和时不进入无界队列而写入跳过诊断，超时会显式终止并回收直接子进程，Unix 还以独立进程组清理仍持有管道的后代。
...
| 触发器 | 已实现 | 多条 contains/regex 规则、多动作编辑、高亮、通知、时间线、本地命令、发送文本、自定义链接和声音均有模型、运行时 dispatch 与回归覆盖；Profile/编辑器/缓存/运行时共享 64 条规则、每条 16 个动作和字段长度/合法性边界，本地命令另有 4 路并发、30 秒总时限、双流各 64 KiB、饱和拒绝诊断及显式进程清理边界。 |
