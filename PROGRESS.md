...
- SSH/Tmux 只按当前 Profile 的 `authOrder` 尝试认证；启用 `recordSuccess` 时，成功方式会随连接状态一起持久化到 `lastSuccessful`，但仅在它仍属于当前顺序时才于下次初连/重连前置。用户移除该方式或关闭记录后，前端草稿、旧 Store 加载和共享 Store 写入都会清除/拒绝越界提示，不会让历史成功方式绕过当前策略。
