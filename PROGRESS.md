...
当前 Rust workspace 自动化测试总数为 159：`portmate` 123、`portmate-kdf` 1、`portmate-core` 24、`portmate-mcp` 11；`npm test` 另有 47 个前端 transfer/selection/presentation/log-shard/workspace/trigger/sync-input/secret-migration 单元测试。
...
| Shell/SSH/Telnet/TCP/Serial | 部分实现 | 基础连接读写、Telnet 增量协商/NVT CR 编解码/TTYPE/raw byte IAC 转义、Telnet/Raw TCP loopback、SSH 与 TCP/Telnet 重连加载最新 Profile/拒绝过期尝试、TCP pending/connected 阶段禁用收敛、虚拟串口 PTY 替换自动重连、runtime 最近断开原因可见、break、DTR/RTS、hex 字节发送、串口最近收发 Hex/时间戳查看可用；Telnet 高级选项、Serial 最新 Profile 重载、深度健康探测和完整 Hex viewer 待补。 |
...
5. 扩展自动重连、断线恢复和连接健康检测：SSH 与 TCP/Telnet 会在重试前加载最新 Profile，并在尝试完成时拒绝已过期配置；TCP/Telnet 在 pending/connected 阶段关闭重连均能立即或下次断线时收敛，Serial 已有初版，runtime 最近断开时间/原因已可见；下一步补 Serial 最新 Profile 重载和更深健康探测。
