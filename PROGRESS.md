...
- 搜索弹窗支持会话和已加载日志搜索。
- MCP grant 管理弹窗、Transfer/Tunnel/Tmux/Trigger 相关入口已存在；Sysmon 已从单行通知升级为 CPU/内存/负载/吞吐概览与进程/磁盘/网络三页工作窗口，并提供当前会话可启停、立即采样后每 10 秒刷新的紧凑工具栏 applet。
...
- Sysmon 旧摘要快照兼容、Linux/macOS CPU/内存/负载解析、Top 进程排序与 8 条边界、磁盘解析/挂载点去重与 16 条边界、Linux `/proc/net/dev` 与 macOS/BSD `netstat -ibn` 每接口采样速率/重复行去重/计数器重置及 32 条边界、完整远端 marker 输出、真实本机 Linux `/proc`/`ps`/`df` 采样，以及 SQLite v3→v4 details 迁移。

当前 Rust workspace 自动化测试总数为 211：`portmate` 150、`portmate-kdf` 1、`portmate-core` 33、`portmate-mcp` 27；`npm test` 另有 65 个前端 transfer/selection/presentation/log-shard/workspace/trigger/sync-input/secret-migration/SSH-health/TCP-health/Serial-health/Serial-capture/proxy 单元测试。
...
| Sysmon | 大部分实现 | 本机 Linux 与 SSH/Tmux Linux/macOS 的 CPU、内存、load average、uptime、聚合吞吐、Top 进程、磁盘和每接口速率/累计量已进入有界快照、SQLite details、MCP resource 和可刷新三页工作窗口；远端先用 `uname -s` 选择有界采样器，不支持的平台明确拒绝；当前会话工具栏 applet 支持立即/10 秒采样、请求去重、断线停止和失败保留旧值；真实 macOS SSH 主机矩阵、FreeBSD、历史趋势与独立常驻侧栏待补。 |
...
4. Sysmon 的进程、磁盘、网络接口、Linux/macOS 远端采样、工作窗口、10 秒工具栏 applet 和结构化持久化已完成；继续补真实 macOS 主机矩阵、FreeBSD、历史趋势与独立常驻侧栏。
