...
- Rust Store 在持久化前同样折叠断线原因空白并限制为 256 个 Unicode 字符，空原因回退到状态默认诊断；加载旧 SQLite/兼容快照时也会归一化历史字段。formatter 单次流式扫描且只保留有界输出，不会为被篡改的超大字段构造完整副本或字符数组；单元回归覆盖多字节字符、精确边界、第 257 字符、超大前导空白和 legacy 加载。
