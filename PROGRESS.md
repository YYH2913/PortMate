...
- Tmux：远端 `list-sessions`、`list-windows`、`list-panes`、attach/new-session，以及按 `session:window` 读取和切换 `synchronize-panes`。管理弹窗按 window 聚合并排序 pane，展示活动 pane/命令/标题，并支持新建 window、重命名 session/window、水平/垂直 split pane、与相邻 pane 交换、四方向按 5 cells 调整、五种内置 window layout，以及经对象确认条关闭 session/window/pane；所有操作都等待 SSH exec 成功并重新读取远端快照后再更新，失败不会保留错误的乐观状态。后端只接受固定 mutation action，target 限制为 1..=256 字符、名称限制为 1..=128 字符、resize amount 限制为 1..=100，文本均拒绝控制字符并经过 shell quoting；带引号/分号的名称不会注入命令，成功 mutation 会写入 session system event。管理器已拆为独立 lazy chunk。
...
- Serial 的断线重开、最新 Profile 重载和过期尝试拒绝已接入；精确 Hex/ASCII、时间戳、方向过滤、增量查询、清空与 JSONL 导出已接入会话侧栏。工具菜单和侧栏可打开真正独立的 Tauri WebviewWindow 串口分析器，浏览器预览降级为同源 popup；窗口自行轮询捕获/连接状态，支持后端读取分片、可跨读取边界但不跨 RX/TX 的 Hex 分隔符、1..4096 字节定长和 1..60000 ms 空闲间隔四种 framing，保留完整/尾帧/截断证据。分析器提供方向/Hex/ASCII/书签筛选、跟随、键盘行导航、100/250/500 分页、4,096 派生帧显示环、完整 Hex dump、绑定原始捕获 ID 的跨解析模式书签、精确源帧导出，以及端口/线路参数/重连中/最近断开原因可视化；仍缺 Modbus/SLIP/COBS 等专用协议解码和突破后端 512 帧/1 MiB 内存环的更大持久数据集。
...
- 串口分析器设置损坏值修复、Hex 分隔符校验、跨捕获分片且方向隔离的 delimiter framing、定长完整/尾帧、空闲间隔、截断传播、4,096 帧环形显示边界、方向/Hex/ASCII/书签筛选、书签上限/不可变切换、Hex dump 和独立窗口路由校验。
...
- 已有基于浏览器 CDP 的工作区、独立窗口和截图回归；终端兼容与 Tmux workflow 已整理为仓库内 `playwright-core` suite，其他一次性 CDP 检查仍待迁移。Tmux workflow 覆盖同步状态聚合、成功开关、失败回滚、刷新、attach/new-session、session/window 新建/重命名/确认关闭、pane split/swap/resize/确认关闭、window layout 及桌面/移动截图边界。
...
| Tmux | 大部分实现 | list/attach/new-session、按 window 聚合的 pane inspection、经 SSH exec 确认的 `synchronize-panes` 开关、session/window 新建/重命名/确认关闭、pane split/swap/resize/确认关闭和五种内置布局可用；跨 window pane join/break/move 与 control-mode 实时事件仍待补。 |
...
7. Tmux session/window/pane 列表、attach/new-session、session/window 新建/重命名/确认关闭、pane split/swap/resize/确认关闭、五种内置布局和 window 级同步输入已完成，并有目标注入/resize 边界与 Playwright workflow 回归；下一步补跨 window pane join/break/move 和 control-mode 实时事件。
...
5. 终端兼容和 Tmux workflow 已整理为仓库内 Playwright 回归，分别覆盖 alternate screen/ANSI/truecolor/宽字符/双 pane resize/SGR mouse/缓存恢复，以及同步开关/失败回滚/attach/session-window lifecycle/pane-layout mutation/桌面移动布局；继续迁移其他 CDP 截图检查，并补完整 vttest 与真实全屏程序矩阵。
