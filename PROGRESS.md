...
`npm run build` 已把应用壳、终端设置、密钥管理器、MCP Bridge、Sysmon 详情、日志管理、低频传输任务弹窗、端口转发弹窗、工作区辅助筛选面板、session/终端右键菜单、搜索、view 右键菜单/重命名弹窗、Quick Command 管理器、命令历史状态、SessionSummary cache 校验、串口分析器/窗口创建器、浏览器终端导出、终端 buffer/选择/在线搜索动作、xterm core/命令目录、WebGL 和 CSS 拆为真实 lazy chunk。当前主 JS 约 418.01 kB、终端设置约 11.58 kB、密钥管理器约 44.88 kB、Sysmon 详情约 11.26 kB、日志管理约 11.20 kB、端口转发弹窗约 5.00 kB、传输任务弹窗约 4.23 kB、SessionSummary cache 校验约 5.45 kB、MCP Bridge 约 13.3 kB、独立终端窗口约 9.12 kB、终端 core JS 约 467.48 kB、WebGL JS 约 120.4 kB、主 CSS 约 140.4 kB、终端 CSS 约 3.9 kB；共享 session 搜索状态约 1.9 kB、工作区辅助筛选约 2.88 kB、命令历史状态约 1.8 kB、session/终端菜单约 4.84 kB、view 右键菜单约 4.60 kB、终端 buffer 约 1.4 kB、选择/在线搜索约 2.2 kB。主包与终端 chunk 均低于 500 kB，没有通过抬高阈值隐藏 warning。
...
当前 Rust workspace 自动化测试总数为 333：`portmate` 253、`portmate-kdf` 1、`portmate-core` 48、`portmate-mcp` 31；`npm test` 另有 56 个文件、314 个前端 desktop-clean/display-formatter/menu-capability/transfer-capability/selection/presentation/log-shard/workspace/workspace-hotkey/workspace-view-context/workspace-panel/workspace-utility/context-menu/terminal-settings/session-settings/session-runtime/session-search/session-cache/screen-lock/detached-pane/trigger/sync-input/terminal-state/terminal-search/terminal-export/terminal-buffer/terminal-selection/terminal-goto-line/terminal-mouse/terminal-theme/command-history/Tmux/free-input/quick-command/OneKey/clipboard/secret-migration/SSH-health/TCP-health/Serial-health/Serial-capture/proxy/Sysmon-history/MCP-audit/MCP-approval/AppImage-runtime/keyed-request-gate 单元测试。OpenSSH/socat/Stronghold/SQLite 集成测试在仓库内默认使用四个 libtest 线程，避免高核心数开发机过度并行造成虚假 wall-clock 超时，显式 `RUST_TEST_THREADS` 仍可覆盖。
