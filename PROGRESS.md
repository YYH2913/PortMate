...
- HTTP MCP 已补 `Accept: application/json, text/event-stream` streamable-http JSON 兼容回归，GET `text/event-stream` 基础事件流、纯 SSE POST 的 `message` 事件响应，以及 Content-Type/协议版本/CORS preflight/严格 framing/重复头/quality value 回归；官方 TypeScript SDK 1.29.0 的真实客户端序列已覆盖。Python `mcp` 1.9.4/1.15.0/1.23.3/1.28.1、官方 Go SDK 1.4.0/1.5.0/1.6.1、官方 Rust SDK `rmcp` 1.0.0/1.1.0/1.8.0/2.2.0、官方 Ruby SDK `mcp` 1.0.0、官方 Java SDK `mcp` 1.0.0/1.1.3/2.0.0、官方 Kotlin SDK `kotlin-sdk-client-jvm` 0.14.0、官方 C# SDK `ModelContextProtocol.Core` 1.4.1 与官方 Swift SDK 0.12.1 使用隔离环境启动同一真实 bridge，并通过 stdio 与无状态 Streamable HTTP 的 8 条消息/请求生命周期，验证 `2024-11-05`/`2025-03-26`/`2025-06-18` 协商及 tools/resources/prompts。Swift 0.12.1 的上游 transport/依赖声明无法在 Windows 原样编译，因此 Swift 矩阵只定义 Linux/macOS；更多 SDK 版本仍待扩展。
...
3. MCP 官方 TypeScript SDK 1.29.0、Python SDK 1.9.4/1.15.0/1.23.3/1.28.1、Go SDK 1.4.0/1.5.0/1.6.1、Rust SDK `rmcp` 1.0.0/1.1.0/1.8.0/2.2.0、Ruby SDK 1.0.0、Java SDK 1.0.0/1.1.3/2.0.0、Kotlin SDK 0.14.0、C# SDK 1.4.1 与 Swift SDK 0.12.1 的 stdio/HTTP 矩阵已纳入回归；继续扩展版本和客户端实现。
...
1. `lib.rs` 已累计减少约 23,900 行，并按以下边界持续拆分；主文件不再直接拥有 Tauri command：
   ...
   - session I/O/logging：`session_commands.rs`、`outbound_io.rs`、`outbound_events.rs`、`session_events.rs`、`sysmon_commands.rs`、`terminal_export_commands.rs`、`trigger_runtime.rs`、`log_commands.rs` 和 `log_storage.rs` 承接 session 查询/输入/resize/open/delete/close 命令适配、每会话出站 lane、跨协议 writer、双向事件、Sysmon/终端导出桌面命令适配、触发动作/受限子进程/发送批次编排、日志/归档桌面命令、append-only shard 存储与管理，以及 bytesRef v1/v2 解析、范围读取和 SHA-256 替换检测。
   ...
   命令适配 ownership 已收敛；下一步继续拆分仍在主文件中的 Sysmon 采集/解析、凭据迁移规划/诊断编排和大型测试夹具。
