...
- 已有基于浏览器 CDP 的工作区、独立窗口和截图回归；终端兼容与 Tmux workflow 已整理为仓库内 `playwright-core` suite，其他一次性 CDP 检查仍待迁移。Tmux workflow 覆盖同步状态聚合、成功开关、失败回滚、刷新、attach/new-session、session/window 新建/重命名/确认关闭、pane activate/split/swap/resize/break/move/确认关闭、move 失败保留快照、window layout 及桌面/移动截图边界。
...
| Tmux | 大部分实现 | list/attach/new-session、按 window 聚合的 pane inspection、经 SSH exec 确认的 `synchronize-panes` 开关、session/window 新建/重命名/确认关闭、pane activate/split/swap/resize/break/move/确认关闭和五种内置布局可用；control-mode 实时事件与服务端推送刷新仍待补。 |
...
7. Tmux session/window/pane 列表、attach/new-session、session/window 新建/重命名/确认关闭、pane activate/split/swap/resize/break/move/确认关闭、五种内置布局和 window 级同步输入已完成，并有 source/destination 注入/resize 边界与成功/失败 Playwright workflow 回归；下一步补 control-mode 实时事件与服务端推送刷新。
...
5. 终端兼容和 Tmux workflow 已整理为仓库内 Playwright 回归，分别覆盖 alternate screen/ANSI/truecolor/宽字符/双 pane resize/SGR mouse/缓存恢复，以及同步开关/失败回滚/attach/session-window lifecycle/pane-layout/跨 window move mutation/桌面移动布局；继续迁移其他 CDP 截图检查，并补完整 vttest 与真实全屏程序矩阵。
