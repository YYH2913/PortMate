...
- Serial 的断线重开、最新 Profile 重载和过期尝试拒绝已接入；精确 Hex/ASCII、时间戳、方向过滤、增量查询、清空与 JSONL 导出已接入会话侧栏。工具菜单和侧栏可打开真正独立的 Tauri WebviewWindow 串口分析器，浏览器预览降级为同源 popup；窗口自行轮询捕获/连接状态，支持后端读取分片、可跨读取边界但不跨 RX/TX 的 Hex 分隔符、1..4096 字节定长、1..60000 ms 空闲间隔、RFC 1055 SLIP 和 COBS 六种 framing。SLIP 使用浏览器兼容的成熟 `slip` 解码器，COBS 使用零依赖且面向浏览器/Web Serial 的 `@serialpilot/cobs` 解码器；两者都跨捕获分片保留状态，逐帧保留解码载荷、原始线上字节和精确源 ID。SLIP 无效转义会标错但不丢失解码结果；COBS 无效/长度截断帧保留完整线上证据而不伪造部分载荷；详情可在解码/线上证据间切换，筛选同时搜索两者。分析器还保留完整/尾帧/截断证据，提供方向/Hex/ASCII/书签筛选、跟随、键盘行导航、100/250/500 分页、4,096 派生帧显示环、完整 Hex dump、绑定原始捕获 ID 的跨解析模式书签、精确源帧导出，以及端口/线路参数/重连中/最近断开原因可视化；仍缺 Modbus 等专用协议解码和突破后端 512 帧/1 MiB 内存环的更大持久数据集。
...
- 串口分析器设置损坏值修复、Hex 分隔符校验、跨捕获分片且方向隔离的 delimiter/SLIP/COBS framing、RFC 1055 END/ESC 解码、COBS 连续/空载荷/长度错误、解码/线上双视图、定长完整/尾帧、空闲间隔、截断传播、4,096 帧环形显示边界、方向/Hex/ASCII/书签筛选、书签上限/不可变切换、Hex dump 和独立窗口路由校验。
...
| Shell/SSH/Telnet/TCP/Serial | 部分实现 | 基础连接读写、SSH/Tmux/TCP/Telnet 的 Profile 级 HTTP CONNECT/SOCKS5 与可选认证、SSH/Tmux 的重连延迟与协议 KeepAlive 阈值、Telnet 增量协商/NVT CR 编解码/Profile TTYPE/方向性 BINARY/NAWS/raw byte IAC 转义、Telnet/Raw TCP loopback、TCP/Telnet 的 Profile 级重连延迟与 OS keepalive、Serial 的重连延迟/无探测接收空闲阈值/精确有界 RX-TX 捕获/方向与内容过滤/原子 JSONL 导出、独立分析窗口/capture/delimiter/fixed/gap/SLIP/COBS framing/解码与线上证据/协议错误/书签/分页/源帧导出/重连诊断、SSH/TCP/Telnet/Serial 重连加载最新 Profile 并拒绝过期尝试、TCP/Telnet/Serial pending/connected 阶段禁用收敛、虚拟串口切换最新端口自动重连、runtime 最近断开原因可见、break、DTR/RTS 和 hex 字节发送可用；Modbus 解码和超过内存环的大数据集待补。 |
...
5. 串口工具增强：精确有界 Hex/ASCII viewer、收发过滤、JSONL + SHA-256 导出、独立 Tauri 分析窗口、capture/delimiter/fixed/gap framing、RFC 1055 SLIP/COBS 解码、解码/线上证据切换、协议错误、分页、书签和重连状态可视化已完成；下一步补 Modbus 专用解码和突破 512 帧/1 MiB 内存环的持久大数据集。
