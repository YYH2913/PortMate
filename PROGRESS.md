...
- HTTP MCP 已补 `Accept: application/json, text/event-stream` streamable-http JSON 兼容回归，GET `text/event-stream` 基础事件流、纯 SSE POST 的 `message` 事件响应，以及 Content-Type/协议版本/CORS preflight/严格 framing/重复头/quality value 回归；官方 TypeScript SDK 1.29.0 的真实客户端序列已覆盖。Python `mcp` 1.9.4/1.15.0/1.23.3/1.28.1、官方 Go SDK 1.6.1、官方 Rust SDK `rmcp` 1.1.0、官方 Ruby SDK `mcp` 1.0.0、官方 Java SDK `mcp` 2.0.0、官方 Kotlin SDK `kotlin-sdk-client-jvm` 0.14.0、官方 C# SDK `ModelContextProtocol.Core` 1.4.1 与官方 Swift SDK 0.12.1 使用隔离环境启动同一真实 bridge，并通过 stdio 与无状态 Streamable HTTP 的 8 条消息/请求生命周期，验证 `2024-11-05`/`2025-03-26`/`2025-06-18` 协商及 tools/resources/prompts。Swift 0.12.1 的上游 transport/依赖声明无法在 Windows 原样编译，因此 Swift 矩阵只定义 Linux/macOS；更多 SDK 版本仍待扩展。
...
3. MCP 官方 TypeScript SDK 1.29.0、Python SDK 1.9.4/1.15.0/1.23.3/1.28.1、Go SDK 1.6.1、Rust SDK `rmcp` 1.1.0、Ruby SDK 1.0.0、Java SDK 2.0.0、Kotlin SDK 0.14.0、C# SDK 1.4.1 与 Swift SDK 0.12.1 的 stdio/HTTP 矩阵已纳入回归；继续扩展版本和客户端实现。
