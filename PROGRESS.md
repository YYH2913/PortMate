...
但它还不是完整 WindTerm/Bitvise 替代品。当前主要差距集中在：GSSAPI、portable-vault/keyring 的系统化原生跨平台故障矩阵、真实 macOS/Windows/FreeBSD SSH 主机与物理串口证据、更多 MCP SDK 版本，以及发布签名/安装后 smoke test。仓库已经补齐可在 Linux Docker/Chrome 自动完成的 vttest、全屏程序、Tmux 多版本、SSH/SFTP/SCP、Telnet/TCP、SSH 健康故障，以及 Go/Rust/Ruby/Java/Kotlin/C#/Swift MCP SDK 矩阵，并新增三平台 native CI 定义。
...
- WindTerm scheme 风格命令补全已接入焦点 Remote 模式的 Shell/SSH/Tmux/Telnet/Raw TCP 主窗口与独立窗口。候选在本地合并常用命令名、选项、子命令、显式历史和 Quick Commands，不探测远端或调用外部服务；`自动补全`设置会独立控制来源、1/2/3 字符触发、5/7/10 行列表和预览方式。方向键选择、Tab 只经现有交互/同步输入 lane 追加尚未输入的后缀、Escape 关闭，绝不自动执行命令。跟踪器支持纯追加输入、Backspace、Ctrl+U/Ctrl+W；bracketed paste、浏览器/系统普通 paste、光标/未知控制序列、引号、重定向和 shell 运算符都会保守暂停到下一行，OneKey 敏感提示出现时也会先抑制候选，避免伪造远端行编辑状态。真实 Chrome 终端回归锁定普通 paste 后候选消失及下一次回车后的恢复。
...
- 发布构建会清理旧 bundle、使用 `0022` umask、规范增量 release binary 与只读资源权限，再生成目标三元组对应的 `portmate-mcp` sidecar，并把主程序、bridge、PNG/ICNS/ICO 标准图标和 Apache-2.0 许可证纳入 Tauri bundle。Linux 构建会保留调用方提供的有效 `LDAI_RUNTIME_FILE`；否则从已有 Tauri AppImage 插件只读解析 type-2 ELF/SquashFS 边界并把 runtime 注入 LinuxDeploy，缓存存在时不再依赖打包阶段访问 GitHub，缓存不存在时仍保留标准下载路径。Linux AppImage 会在复用原 runtime 的前提下重新封装，消除 LinuxDeploy 写入的开发机绝对 `.DirIcon` 并把目录/可执行/普通文件规范为 `0755/0755/0644`。仓库脚本会解包核对 DEB、RPM、AppImage 的主程序、sidecar、desktop entry、图标、许可证、全部文件权限与 symlink 边界，并把精确生产 CSP 和主/独立窗口 capability source policy 与每个包内主程序的嵌入元数据交叉验证；每个包内 bridge 还分别通过官方 SDK 的 stdio 8 消息和 HTTP 9 请求序列。真实 AppImage 已在隔离 XDG 目录连续启动两次并通过标准窗口关闭退出：SQLite/兼容 JSON 的 SHA-256 跨重启保持不变，IPC 地址和规范 UUID tokenRef 每次轮换，endpoint 与本轮 Secret Service 条目在两次正常退出后都立即消失，且 Unix endpoint/lock 权限均为 `0600`。MCP HTTP 页返回已安装 bridge 的真实路径、精确 Store 路径和不含 token/keyring ref 的平台 shell 启动命令，不再要求最终用户在源码目录执行 `cargo run`，也不会再因缺少 `PORTMATE_STORE_PATH` 启动一个无法连接桌面的 bridge。bundle identifier 已从与 macOS `.app` 扩展冲突的 `dev.portmate.app` 改为 `dev.portmate.desktop`；首次启动只会把旧数据目录原子迁移到不存在或空的新目录，两个非空目录并存时拒绝猜测合并。生产 WebView 已启用限制 script/connect/frame/object/base/form 的 CSP，开发 HMR 继续使用独立 dev CSP。
...
- SSH PTY shell：`russh` 连接、PTY、resize、password/public-key/keyboard-interactive/ssh-agent、profile-vault 私钥、保存密码/口令。每个 host-key 扫描跳点的完整认证顺序共用 12 秒总预算，每个正式 Jump Host/目标认证共用生产环境 20 秒 setup 预算；目标认证后的 session channel/PTY/环境/agent forwarding/shell/Tmux attach 另共用一个 20 秒总预算。已连接 runtime 的 SFTP 初始化，以及 SCP/remote copy/Tmux control/tunnel 健康/Sysmon 共用的 channel + exec setup 限制为 10 秒，短时辅助命令会把 setup 计入自身总预算。超时或 runtime 注册前 setup 失败会有界断开当前 handle 和已经建立的 Jump Host 链。
...
- 活动 SSH/Tmux runtime 可执行三层健康检查：3 秒 keepalive 往返决定 transport 是否 `unresponsive`，5 秒 exec channel 必须返回独立 marker，可选 SFTP 初始化失败则标记 `degraded`；全部请求阶段成功才为 `healthy`。结果携带三段独立 latency/error 并绑定开始时的 runtime ID，检查期间重连或替换会 fail-closed 要求重试。OpenSSH fault server 矩阵真实覆盖容器暂停导致 ping 无响应、exec 拒绝/静默、SFTP subsystem 缺失/拒绝，以及检查期间 runtime 代际替换。
...
- SSH tunnel：local、remote reverse、dynamic SOCKS5，桌面端可查看当前会话运行中的 tunnel、停止 tunnel、显示 active/total 连接数、双向字节计数和最后错误；2 秒状态读取按对话框代际串行化，关闭/换会话/新建/停止会失效旧响应，慢请求不能回滚当前列表。dynamic SOCKS5 的 method + CONNECT request 使用单个 5 秒总 deadline，半包/慢速客户端会关闭并进入 tunnel 错误指标，domain target 还会拒绝空值、非 UTF-8、控制字符和空白字符。local/dynamic 的 handle lock 与 `direct-tcpip` open、remote reverse 的本地目标连接分别限制为 10 秒；russh channel open 超时时会先请求断开 SSH session，清理由已取消确认 future 留下的潜在孤儿 channel，再由既有断线/重连路径收敛。三种模式共用全应用 256 个活动客户端槽位，饱和 local socket 在创建任务前关闭，饱和、无路由或服务端端口超出 `u16` 的 remote-forward channel 走 1 秒有界 Close，不会把 `u32` 端口回绕到其他路由。remote reverse 本地目标连接失败同样使用该有界收尾。local/dynamic 使用端口 0 时会回填实际监听端口；目标失败会记录错误，后续连接成功会清除 degraded 状态，监听器永久退出会从运行 registry 移除并禁用已保存配置；remote forward 每 15 秒通过远端 Linux `/proc/net/tcp`/`ss`、FreeBSD `sockstat`、macOS `lsof` 或成功执行的 `netstat -ltn` 被动核对监听端口，服务端撤销后会重发原 bind request 并记录恢复事件；存在但参数不兼容的探测工具会回退为 unsupported，不会把空输出误判为监听丢失；Stop 在服务端 cancel 拒绝/超时后仍会清理本地路由/runtime 并把 profile 置为 disabled；SSH channel 断开会移除该会话全部旧 tunnel runtime，自动重连成功后从最新 profile 按原 ID、标签和端口逐条恢复 enabled tunnel，单条恢复失败会保留期望状态、记录事件且不阻断会话和其他 tunnel。
...
- GSSAPI 仍未实现：当前固定的 `russh` 0.60 客户端 API 没有 GSSAPI exchange。完成它需要引入/评审另一条 SSH transport 或等待上游客户端支持，不能用 UI 标记代替协议实现。
...
- ZModem 当前限制单文件小于 4 GiB；OpenSSH PTY 上的 lrzsz X/Y/Z 双向传输已有实测，静默 modem 等待可在取消后立即发送 CAN 并快速退出，transport 进入重连态会立即失败旧 worker；OpenSSH Alpine 3.19/3.20/3.21、Debian Bookworm、Debian trixie、Ubuntu 24.04 和 Dropbear Alpine 七服务端已在独立 SSH runtime 上交错验证 X/Y/ZModem 活动断线，记录部分进度、失败诊断、最终文件未提交和 `.portmate-part` 清理。物理串口、批量和不同工具实现的兼容矩阵仍待补。
...
- HTTP MCP 已补 `Accept: application/json, text/event-stream` streamable-http JSON 兼容回归，GET `text/event-stream` 基础事件流、纯 SSE POST 的 `message` 事件响应，以及 Content-Type/协议版本/CORS preflight/严格 framing/重复头/quality value 回归。官方 TypeScript SDK 1.10.0/1.20.0/1.29.0、Python `mcp` 1.9.4/1.15.0/1.23.3/1.28.1、官方 Go SDK 1.4.0/1.5.0/1.6.1、官方 Rust SDK `rmcp` 1.0.0/1.1.0/1.8.0/2.2.0、官方 Ruby SDK `mcp` 0.25.0/1.0.0、官方 Java SDK `mcp` 1.0.0/1.1.3/2.0.0、官方 Kotlin SDK `kotlin-sdk-client-jvm` 0.12.0/0.13.0/0.14.0、官方 C# SDK `ModelContextProtocol.Core` 1.0.0/1.2.0/1.4.1 与官方 Swift SDK 0.11.0/0.12.1 使用隔离环境启动同一真实 bridge，并通过 stdio 与无状态 Streamable HTTP 生命周期，验证 `2024-11-05`/`2025-03-26`/`2025-06-18` 协商及 tools/resources/prompts。Swift 两版本的上游 transport/依赖声明无法在 Windows 原样编译，因此 Swift 矩阵只定义 Linux/macOS；更多 SDK 版本仍待扩展。
...
`npm run build` 已把应用壳、文件管理器、会话设置、终端设置、密钥管理器、MCP Bridge、Sysmon 详情、日志管理、低频传输任务弹窗、端口转发弹窗、工作区辅助筛选面板、session/终端右键菜单、搜索、view 右键菜单/重命名弹窗、Quick Command 管理器、命令历史状态、SessionSummary cache 校验、串口分析器/窗口创建器、浏览器终端导出、终端 buffer/选择/在线搜索动作、xterm core/命令目录、WebGL 和 CSS 拆为真实 lazy chunk。当前主 JS 约 359.79 kB、文件管理器约 17.23 kB、会话设置约 38.54 kB、终端设置约 11.60 kB、密钥管理器约 44.95 kB、Sysmon 详情约 11.26 kB、日志管理约 11.23 kB、端口转发弹窗约 5.00 kB、传输任务弹窗约 4.27 kB、SessionSummary cache 校验约 5.45 kB、MCP Bridge 约 13.3 kB、独立终端窗口约 9.15 kB、终端 core JS 约 467.51 kB、WebGL JS 约 120.4 kB、主 CSS 约 140.4 kB、终端 CSS 约 3.9 kB；共享 session 搜索状态约 1.9 kB、工作区辅助筛选约 2.87 kB、命令历史状态约 1.8 kB、session/终端菜单约 4.84 kB、view 右键菜单约 4.63 kB、终端 buffer 约 1.4 kB、选择/在线搜索约 2.2 kB。主包与终端 chunk 均低于 500 kB，没有通过抬高阈值隐藏 warning。
...
仓库内 `npm run test:workspace-ui` 会自行启动隔离 Vite/Chrome，从旧版六面板全开快照验证终端优先迁移，并覆盖左/右/底部停靠区同时显示、同区工具标签切换和非活动内容折叠、标题栏聚焦与跨区拖放、三向 pointer/键盘尺寸调整、双击复位、刷新持久化、关闭后空间归还、唯一资源树/历史筛选、四组顶层菜单及 SSH/Serial 精确禁用态、低频 pane 动作从标题栏迁到 view 右键菜单、四方向新分组子菜单、同步输入正反切换、顶部会话/日志共享搜索与键盘激活、发送区无伪控件、真实 Profile 启动目标的禁用/选择/保存/恢复、Profile 删除确认/后端单次调用/view 与快照收敛、终端设置单层标签、六协议会话设置双选择器/协议感知传输/无伪偏好写入、MCP 三任务页互斥/联合审计筛选/详情/精确导出、关闭失败保留连接状态与诊断，以及 1440x900/390x844 桌面移动截图边界；上述非输入操作还会确认没有调用 `send_text`、`send_bytes` 或 `run_command`。其他工作区回归覆盖 v4 view 复制/别名/着色/重载/关闭恢复、同组排序、跨组拖放、独立窗口往返、标签循环、完整 view 右键菜单、分屏方向、精确移组、视图生命周期快捷键、可配置键盘模式、面板显隐、专注模式和主/独立窗口锁屏。命令补全覆盖 Quick/历史/参数排序、Tab 精确后缀、控制序列退让和主/独立窗口一致性；命令历史覆盖旧数组迁移、配置裁剪、发送记录、清除、关闭持久化后的进程内保留和 detached 读取；Quick Commands 覆盖管理/排序/取消/保存、Quick Bar 和插入/执行差异；串口分析器覆盖 delimiter/SLIP/COBS/Modbus framing、实时/Raw 日志源、证据视图、协议错误、分页、书签、重连诊断和导出。
...
- Linux Docker SSH 服务端矩阵已在 OpenSSH Alpine 3.19/3.20/3.21、OpenSSH Debian Bookworm、OpenSSH Debian trixie、OpenSSH Ubuntu 24.04 和 Dropbear Alpine 上真实验证 SFTP/SCP 活动断线，并在同一容器的独立 SSH runtime 上交错验证 X/Y/ZModem 活动断线；Alpine 的 lrzsz 从固定源码以 legacy C 模式构建，YModem 分配到 Debian、Debian trixie 和 Ubuntu（Alpine musl 的 `rb` 不发起 CRC 请求），因此该结果同时记录服务端和工具链边界。
...
- Unicode 11、Serialize、write-only OSC 52、Profile 终端背景不透明度（透明度下 DOM renderer、100% 下 WebGL、原位保留 XTerm/buffer）和 WebGL fallback 已有浏览器回归；alternate screen/ANSI/truecolor/宽字符、双 pane PTY resize owner、SGR mouse、选择复制偏好、缓存恢复、真实 Vim/less/top PTY 和 6,000 行长日志已有可重复 Playwright 基线。新增 vttest 三版本×13 套件、四发行版 Vim/less/top/dialog 和 tmux 3.1c/3.3a/3.5a 矩阵；原生 macOS/Windows 全屏程序及签名安装包 smoke test 仍需对应 runner/设备证据。
...
| SSH | 部分实现 | PTY、密码、公钥、keyboard-interactive、ssh-agent、认证/终端 setup 总 deadline、Profile 级协议 KeepAlive 阈值、带可选认证的 HTTP CONNECT/SOCKS5、多跳 Jump Host 后端连接链路、每跳独立 secretRef/identityRef 和基础编辑可用；代理与 host-key 扫描路径一致且只作用于第一物理跳。两跳 OpenSSH direct-tcpip、三端独立 identity、逐跳 TOFU、连接拒绝、三段握手超时、逐端认证失败、第二跳 key mismatch、混合认证链、真实 ssh-agent 矩阵，以及 keepalive/exec/SFTP 三层健康和六种服务端故障均已端到端覆盖；当前 `russh` 客户端不支持 GSSAPI。 |
...
| MCP HTTP | 部分实现 | `portmate-mcp --http` 支持 loopback JSON-RPC、Origin 校验、Bearer/X-Token、本地 keyring token、无状态 Streamable HTTP、GET SSE、纯 SSE POST、严格 HTTP framing、容量和 deadline；官方 TypeScript SDK 1.10.0/1.20.0/1.29.0、Python 1.9.4/1.15.0/1.23.3/1.28.1、Go 1.4.0/1.5.0/1.6.1、Rust `rmcp` 1.0.0/1.1.0/1.8.0/2.2.0、Ruby 0.25.0/1.0.0、Java 1.0.0/1.1.3/2.0.0、Kotlin 0.12.0/0.13.0/0.14.0、C# 1.0.0/1.2.0/1.4.1 和 Swift 0.11.0/0.12.1 的 HTTP/stdio 生命周期已通过；Swift 受上游限制只定义 Linux/macOS 矩阵。桌面授权/HTTP/审计任务页、请求门控和有界审计导出已有；更多 SDK 版本待扩展。 |
| 测试体系 | 部分实现 | Rust workspace 与前端单测、终端/Tmux/workspace Playwright、vttest 三版本×13 套件、四发行版全屏程序、tmux 3.1c/3.3a/3.5a、OpenSSH 9.6/9.7/9.9 + Debian bookworm + Debian trixie + Ubuntu 24.04 OpenSSH + Dropbear 的正常路径和七例交错 SFTP/SCP 活动上传断线、六种 SSH 健康故障、BusyBox Alpine 3.19/3.21、inetutils Debian/Ubuntu Telnet、Ncat/Socat Alpine/Ubuntu TCP、TypeScript MCP 三版本、Python MCP 四版本、Go MCP 三版本、Rust MCP 四版本、Ruby MCP 双版本、Java MCP 三版本、Kotlin MCP 三版本、C# MCP 三版本和 Swift MCP 双版本均已自动化；DEB/RPM/AppImage 的主程序元数据、资源/权限/链接及包内 sidecar 全语言矩阵已逐包通过。所有 Docker 矩阵支持三次镜像构建重试和显式、缺镜像即失败的离线缓存模式。`Native CI` 定义 Ubuntu/Windows/macOS 测试、Clippy、TypeScript/Python/Go/Rust/Ruby/Java/Kotlin/C#/Swift SDK 和 Tauri bundle，以及 Linux 全兼容矩阵；仍需 CI 实际成功记录、签名安装包 smoke test、真实远端 OS/物理设备证据。 |
...
4. SFTP/SCP 的 OpenSSH Alpine 3.19/3.20/3.21、Debian bookworm、Debian trixie、Ubuntu 24.04 和 Dropbear 正常矩阵、交错的 SFTP/SCP 活动上传断线、SSH health 的 ping/exec/SFTP 故障注入，以及 BusyBox Alpine 3.19/3.21、inetutils Debian/Ubuntu Telnet、Ncat/Socat Alpine/Ubuntu TCP 已完成；SSH 容器就绪会分别等待 Docker 动态端口发布和完整协议 banner，TCP/Telnet 通过容器内 LISTEN 状态检查且不会以裸连接消费被测会话，Telnet shell 用例会先确认首轮 negotiation reply 再发送应用数据，避免冷启动竞态把服务端兼容性与测试控制面故障混为一谈。继续补 modem 物理串口、更多 SSH 服务端实现和异常状态码。
...
7. Tmux 完整管理和 control-mode workflow 已完成，真实 tmux 3.1c/3.3a/3.5a server/control 矩阵也已覆盖；下一步只保留原生 OS/发行版扩展与安装后全屏 smoke test。
...
3. MCP 官方 TypeScript SDK 1.10.0/1.20.0/1.29.0、Python SDK 1.9.4/1.15.0/1.23.3/1.28.1、Go SDK 1.4.0/1.5.0/1.6.1、Rust SDK `rmcp` 1.0.0/1.1.0/1.8.0/2.2.0、Ruby SDK 0.25.0/1.0.0、Java SDK 1.0.0/1.1.3/2.0.0、Kotlin SDK 0.12.0/0.13.0/0.14.0、C# SDK 1.0.0/1.2.0/1.4.1 与 Swift SDK 0.11.0/0.12.1 的 stdio/HTTP 矩阵已纳入回归；继续扩展版本和客户端实现。
...
1. `lib.rs` 已累计减少约 30,000 行，生产实现约在前 3,560 行内，剩余体量主要是同文件测试模块；主文件不再直接拥有 Tauri command，并按以下边界持续拆分：
   - transport/runtime：`tmux_commands.rs`、`tmux_protocol.rs`、`tmux_runtime.rs`、`telnet_protocol.rs`、`proxy_protocol.rs`、`tcp_transport.rs`、`serial_commands.rs`、`serial_transport.rs`、`shell_transport.rs`、`ssh_transport.rs`、`ssh_exec.rs`、`ssh_runtime.rs`、`ssh_tunnel.rs`、`tunnel_commands.rs` 和 `serial_capture.rs` 承接 Tmux/Serial/tunnel 桌面命令适配、Tmux control watcher 注册/容量/事件循环/取消以及状态读取/变更编排、协议状态机、TCP/Telnet/Serial runtime、Shell PTY/子进程 runtime、SSH 正式 handler/host-key 接受/remote-forward 分发、socket/Jump Host/认证/terminal 与 auxiliary exec setup、共享 exec capture/PowerShell 编码、SSH channel reader/断开诊断/代际重连与安装补偿、local/remote/dynamic tunnel runtime/metrics/健康恢复/Profile 补偿与数据 pipe、端口打开与读循环、最新 Profile 自动重连、串口线制映射与 capture。
   - security/profile：`ssh_security.rs`、`ssh_host_key_commands.rs`、`ssh_identity_commands.rs`、`ssh_host_key_scan.rs`、`ssh_health.rs`、`one_key_commands.rs`、`one_key_prompt.rs`、`one_key_runtime.rs`、`profile_commands.rs`、`profile_normalization.rs` 和 `profile_security.rs` 承接 SSH 信任、完整 Host Key 命令/Store mirror/并发字段合并、identity/agent 枚举/更新/轮换/删除、SSH/Tmux/代理 Profile 访问、Client Identity 合并与 Secret 引用规范化/全局计数、直连/多跳扫描、健康检查、OneKey CRUD/发送/提示复核/摘要/登录凭据/身份克隆/Secret 生命周期，以及 Profile 保存事务/归一化。
   - storage/vault：`state_snapshot.rs`、`sqlite_schema.rs`、`sqlite_mirror.rs`、`sqlite_store.rs`、`store_normalization.rs`、`store_persistence.rs`、`store_transactions.rs`、`migration_types.rs`、`migration_planning.rs`、`migration_runtime.rs`、`migration_diagnostics.rs`、`migration_journal_store.rs`、`migration_recovery.rs`、`secret_commands.rs`、`secret_provider.rs`、`vault_commands.rs` 和 `portable_vault.rs` 承接 Store/SQLite/Stronghold/keyring 的持久化、Secret/Vault/迁移命令、copy-on-write/tracked-event 事务、提交后核验、一致性、迁移私有载荷/计划/投影/事务执行/恢复、journal 校验/读写/pending 门禁以及受限诊断模型/生成/原子导出。
   ...
   - transfer：`transfer_commands.rs`、`transfer_runtime.rs`、`file_commands.rs`、`file_operations.rs`、`file_batch.rs`、`file_transfer.rs`、`scp_protocol.rs` 和 `modem_transfer.rs` 承接传输完整桌面命令适配、文件桌面命令适配、共享本地/SFTP 列表/属性/路径安全以及创建/删除/移动/重命名/chmod 编排、队列、批次规划、Local/SFTP/SCP/remote-copy 与 X/Y/ZModem 数据面。
   - session I/O/logging：`session_commands.rs`、`outbound_io.rs`、`outbound_events.rs`、`session_events.rs`、`system_event_sink.rs`、`sysmon_commands.rs`、`sysmon_runtime.rs`、`terminal_export_commands.rs`、`trigger_runtime.rs`、`log_commands.rs` 和 `log_storage.rs` 承接 session 查询/输入/resize/open/delete/close 命令适配、每会话出站 lane、跨协议 writer、双向事件、system event notifier/后台 outbox drain/关闭收尾、Sysmon 桌面命令以及本机/远端采集、四平台解析和网络地址回退、终端导出桌面命令适配、触发动作/受限子进程/发送批次编排、日志/归档桌面命令、append-only shard 存储与管理，以及 bytesRef v1/v2 解析、范围读取和 SHA-256 替换检测。
   ...
   命令适配 ownership、session lifecycle、串口线路控制/Break 事务、MCP 审计导出、Tmux control runtime、system event sink 与 Sysmon 采集/解析边界已收敛；下一步继续拆分仍在主文件中的共享 state/bootstrap 编排和大型测试夹具。
