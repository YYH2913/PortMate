...
`npm run build` 已把应用壳、MCP Bridge、Sysmon 详情、低频传输任务弹窗、端口转发弹窗、工作区辅助筛选面板、session/终端右键菜单、搜索、view 右键菜单/重命名弹窗、Quick Command 管理器、命令历史状态、SessionSummary cache 校验、串口分析器/窗口创建器、浏览器终端导出、终端 buffer/选择/在线搜索动作、xterm core/命令目录、WebGL 和 CSS 拆为真实 lazy chunk。当前主 JS 约 483.79 kB、Sysmon 详情约 11.26 kB、端口转发弹窗约 5.00 kB、传输任务弹窗约 4.23 kB、SessionSummary cache 校验约 5.45 kB、MCP Bridge 约 13.3 kB、独立终端窗口约 9.12 kB、终端 core JS 约 467.43 kB、WebGL JS 约 120.4 kB、主 CSS 约 140.4 kB、终端 CSS 约 3.9 kB；共享 session 搜索状态约 1.9 kB、工作区辅助筛选约 2.88 kB、命令历史状态约 1.8 kB、session/终端菜单约 4.84 kB、view 右键菜单约 4.60 kB、终端 buffer 约 1.4 kB、选择/在线搜索约 2.2 kB。主包与终端 chunk 均低于 500 kB，没有通过抬高阈值隐藏 warning。
