...
- `npm run desktop:clean` 除清理 snap 注入的 GTK/WebKit 环境外，会读取 Tauri 的实际 dev URL，识别并终止工作目录和命令行都属于当前仓库的旧 Vite 监听者；端口属于其他程序或无法验证时只报告 PID 并拒绝误杀。清理后的 XDG 数据目录优先恢复 snap 注入前的原值，否则按当前 `HOME` 和系统共享目录构造，不含开发者绝对路径。`1420` 旧进程阻断已用真实 Tauri 启动链路复现，修复后 Vite 启动、Rust `951/951` 编译和 `target/debug/portmate` 运行均通过。
