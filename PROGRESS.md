...
1. `lib.rs` 已累计减少约 18,100 行，并按以下边界持续拆分：
   - transport/runtime：`tmux_protocol.rs`、`telnet_protocol.rs`、`proxy_protocol.rs`、`tcp_transport.rs`、`serial_transport.rs`、`shell_transport.rs`、`ssh_runtime.rs` 和 `serial_capture.rs` 承接协议状态机、TCP/Telnet/Serial runtime、Shell PTY/子进程 runtime、SSH channel reader/断开诊断/代际重连与安装补偿、端口打开与读循环、最新 Profile 自动重连、串口线制映射与 capture。
   ...
   下一步继续迁出 SSH 连接、认证与终端 setup ownership。
