...
- `xterm.js` 终端渲染已接入 FitAddon、SearchAddon、WebLinksAddon、Unicode11Addon、SerializeAddon、ClipboardAddon 和按需加载的 WebglAddon。`终端 -> 查找`、`Ctrl/Cmd+F` 和 WindTerm 终端默认键 `Ctrl+Shift+F` 会在当前焦点 pane 打开增量查找条，支持前后匹配、计数、大小写、整词和正则模式；`终端 -> 跳转到行` 已从错误的日志搜索路由改为当前焦点 XTerm buffer 的本地绝对/相对行跳转，输入实时居中预览，Escape 恢复原 viewport，Enter 提交且不向远端发送文本，越界目标拒绝。Profile 终端设置提供 PortMate 深色、石墨、Solarized 深色和 PortMate 浅色四套受控主题；字体、字号、滚屏和主题保存后会原位更新同一 Profile 的已挂载 view，不重建 XTerm 或丢失 buffer，未知旧主题在前后端都回退默认值。WebGL 初始化失败或 context loss 会回退 DOM renderer；OSC 52 只允许写系统剪贴板，远端读取固定返回空内容。
...
| 测试体系 | 部分实现 | 269 项 Rust core/协议集成测试、53 文件/273 项前端单测和仓库内终端/Tmux/workspace UI Playwright 基线可用；终端基线已加入真实 Vim/less/top PTY 和有界 6,000 行日志性能回归，workspace 基线覆盖 Profile 主题选择、原位应用和桌面截图，其他 UI 检查迁移、完整 vttest、真实 tmux 和跨平台矩阵仍不足。 |
