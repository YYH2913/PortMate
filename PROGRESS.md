...
- `npm run desktop:clean` 除清理 snap 注入的 GTK/WebKit 环境外，会读取 Tauri 的实际 dev URL，并在 Linux/macOS/Windows 上通过 OS 监听与进程命令信息识别当前仓库的旧 Vite；端口属于其他程序或无法验证时只报告 PID 并拒绝误杀。Unix 清理后的 XDG 数据目录优先恢复 snap 注入前的原值，否则按当前 `HOME` 和系统共享目录构造，不含开发者绝对路径；Windows 保留完整构建/签名环境并只移除已知 Snap/GTK 注入项。`1420` 旧进程阻断已用真实 Linux Tauri 启动链路复现，Windows `netstat` listener、当前/其他 checkout 命令边界和环境保留使用无副作用回归锁定。
