...
1. `lib.rs` 已累计减少约 14,400 行，并按以下边界持续拆分：
   ...
   - bundle security：`bundle_signing.rs` 承接 keyring/Portable Vault 签名身份 failover、read-back 核验、Ed25519 detached signature 和多产物 fail-closed 收敛。

   下一步继续拆分 bundle archive/attachment orchestration 与通用 archive support。
