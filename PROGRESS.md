...
- SSH PTY shell：`russh` 连接、PTY、resize、password/public-key/keyboard-interactive/ssh-agent、profile-vault 私钥、保存密码/口令。每个 host-key 扫描跳点的完整认证顺序共用 12 秒总预算，每个正式 Jump Host/目标认证共用生产环境 20 秒 setup 预算；超时会有界断开当前 handle 和已经建立的 Jump Host 链。
