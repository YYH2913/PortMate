...
- HTTP MCP 已补 `Accept: application/json, text/event-stream` streamable-http JSON 兼容回归，GET `text/event-stream` 基础事件流、纯 SSE POST 的 `message` 事件响应，以及 Content-Type/协议版本/CORS preflight/严格 framing/重复头/quality value 回归；官方 TypeScript SDK 1.29.0 的真实客户端序列已覆盖。Python `mcp` 1.9.4/1.28.1、官方 Go SDK 1.6.1、官方 Rust SDK `rmcp` 1.1.0、官方 Ruby SDK `mcp` 1.0.0、官方 Java SDK `mcp` 2.0.0、官方 Kotlin SDK `kotlin-sdk-client-jvm` 0.14.0、官方 C# SDK `ModelContextProtocol.Core` 1.4.1 与官方 Swift SDK 0.12.1 使用隔离环境启动同一真实 bridge，并通过 stdio 与无状态 Streamable HTTP 的 8 条消息/请求生命周期，验证 `2024-11-05`/`2025-06-18` 协商及 tools/resources/prompts。Swift 0.12.1 的上游 transport/依赖声明无法在 Windows 原样编译，因此 Swift 矩阵只定义 Linux/macOS；更多 SDK 版本仍待扩展。
...
1. `tmux_protocol.rs`、`telnet_protocol.rs`、`proxy_protocol.rs` 已承接对应协议状态机，`tcp_transport.rs` 已完整承接 TCP/Telnet runtime 所有权、端点归一化、代理 socket 建立、TCP_NODELAY/keepalive、初次安装/失败回收、增量读循环、Telnet 协商回复、断线状态收敛，以及读取最新 Profile、代际校验、延迟轮询和原子安装的自动重连状态机；`serial_capture.rs` 迁出有界 Serial capture 模型、增量快照和 session registry；`ssh_security.rs` 进一步迁出 ssh-agent signer/auth、一次性信任、TOFU/Host Key 持久化、`lastSeen` 和 Profile 镜像同步，`ssh_health.rs` 迁出三层健康命令与报告；`one_key_prompt.rs` 迁出终端转义清洗、凭据提示识别、事件时效和字段匹配验证；`profile_normalization.rs` 迁出终端/Profile 元数据、旧连接默认值和 trust/auth 归一化；`state_snapshot.rs` 迁出 Store/Portable Stronghold 文件锁、SHA-256 版本、跨实例陈旧写入防护和 Stronghold 提交/换密 guard；`sqlite_schema.rs` 迁出 SQLite 表、索引、schema version 和兼容列迁移；`sqlite_mirror.rs` 迁出 Profile/runtime 全量镜像和 event/audit/timeline/Sysmon 增量协调；`sqlite_store.rs` 迁出 SQLite load、FULL-sync transaction、migration checkpoint、revision 和提交后读回验证；`migration_journal_store.rs` 迁出凭据迁移 journal 状态转换、原子 metadata 写入和读回核对；`migration_recovery.rs` 迁出 before/after 投影判定、冲突冻结、目标回滚和源清理；`secret_provider.rs` 迁出 native keyring 初始化、Native/Portable 路由、统一 secret CRUD 和迁移 provider 批处理；`portable_vault.rs` 迁出 Stronghold context、salt/KDF/open/rekey、单条/批量 CRUD 和失败提交回滚；`store_normalization.rs` 迁出 legacy JSON load、Profile/session remap、孤儿裁剪、MCP/OneKey 归一化、mirror key 修复、运行时/传输中断收敛和历史上限；`store_persistence.rs` 迁出 Store 初始化、CAS 版本缓存、journal metadata 事务、SQLite/JSON 写协调和提交后指纹核对；`mcp_control.rs` 迁出 grant 上限/归一化/增删策略、MCP HTTP sidecar 定位/shell quoting/loopback 启动配置，以及 approval 模型、32 项 pending map、60 秒 fail-closed、one-shot 响应、排序和 UI emitter；`mcp_ipc.rs` 迁出私有端点文件校验/原子发布、旧 token 回收、64 项连接上限、超时/有界 framing、常量时间 token 校验和连接响应生命周期；`mcp_authorization.rs` 迁出读写 scope 映射、client/session 标识校验、trusted bootstrap/confirm-writes 判定、写前 fail-closed 审计、桌面审批编排，以及已执行结果的持久化核验和降级标记；`mcp_execution.rs` 迁出具体工具分发、JSON 参数读取、跨命令结果序列化和 session/event/transfer/Tmux 返回值脱敏；`transfer_runtime.rs` 迁出取消令牌、进度上下文、路径/default-dir 策略、协议启用复核、全局/每会话容量、串行 lane、排队/运行/完成/取消持久化、速率限制和重试编排；`file_batch.rs` 迁出本地/远端递归批次规划、符号链接/重复/根路径防护、冲突 fail/skip/overwrite/rename、目标预检、目录准备、容量预留和逐文件 transfer 入队；`file_transfer.rs` 迁出 Local/SFTP/SCP 路由、SSH auxiliary/SFTP session lease、SFTP setup/request 超时、断点文件与前缀核对、SFTP 上传/下载/远端复制、路径/符号链接保护，以及带总/空闲超时和单调 marker 校验的 SSH remote-copy；`scp_protocol.rs` 迁出有界 channel 读写、取消/空闲超时、ACK/文件头/行长 framing、上传续传 SHA-256 握手、源增长核验、下载正文和 EOF/exit-status 完成收敛；`modem_transfer.rs` 迁出 X/Y/ZModem 路由和数据面、远端自动启动与 marker 生命周期、packet/CRC/ACK/NAK/EOT 状态机、重试/超时/取消，以及本地临时输出与 padding 收敛；`outbound_io.rs` 迁出每会话串行 lane、SSH/Shell/TCP/Serial writer 选择、运行时代际校验、Telnet text/bytes wire 编码和 Modem 原始帧写入；`outbound_events.rs` 迁出用户/控制发送编排、OneKey 发送前二次校验、active command 生命周期、raw/text/jsonl 日志降级、Store 持久化和桌面事件广播；`session_events.rs` 迁出 inbound 字节事件、command 关联、运行时代际过滤、触发器派发、system event 发布，以及 raw/text/jsonl 事件写入协调。`lib.rs` 已累计减少约 13,300 行，下一步继续拆分 log shard storage/management。
