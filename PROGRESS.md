...
当前 Rust workspace 自动化测试总数为 217：`portmate` 156、`portmate-kdf` 1、`portmate-core` 33、`portmate-mcp` 27；`npm test` 另有 101 个前端 transfer/selection/presentation/log-shard/workspace/workspace-hotkey/detached-pane/trigger/sync-input/secret-migration/SSH-health/TCP-health/Serial-health/Serial-capture/proxy/Sysmon-history 单元测试。
...
| WindTerm 风格工作台 | 大部分实现 | 主布局和菜单、最多 16 pane/8 层的递归水平/垂直分屏、每组最多 32 个 view、v1/v2 迁移、view 跨组移动/整组合并/四方向新分组、可调且持久化的比例、pane/active/tab color 恢复、可配置且支持最多两段 chord/冲突校验的 WindTerm 分屏/方向焦点/关闭/zoom 快捷键、方向 pane 交换、pane 独立 Tauri 窗口/返回、启动会话策略及 xterm/CSS lazy chunk 已有。 |
...
2. pane view group、启动自动连接、标签颜色、workspace restore、v1/v2 自动迁移、view 跨组移动/整组合并/四方向新分组、任意递归嵌套分屏、pane 独立 Tauri 窗口/返回，以及支持最多两段 chord 的可配置 WindTerm 分屏创建/方向焦点/关闭/zoom 快捷键和方向 pane 交换已完成；下一步补 view 关闭/恢复和更完整的标签管理。
