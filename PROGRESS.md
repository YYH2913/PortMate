...
1. `lib.rs` 已累计减少约 30,000 行，生产实现约在前 3,560 行内，剩余体量主要是同文件测试模块；主文件不再直接拥有 Tauri command，并按以下边界持续拆分：
   ...
   命令适配 ownership、session lifecycle、串口线路控制/Break 事务、Tmux control runtime、system event sink 与 Sysmon 采集/解析边界已收敛；下一步继续拆分仍在主文件中的共享 state/bootstrap 编排和大型测试夹具。
