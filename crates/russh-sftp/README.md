...
This local copy stays at upstream version 2.1.2 and accepts the standard SFTP v4-v6 status codes plus unknown status values. The patch prevents otherwise valid server responses, including Apache MINA SSHD's `FILE_ALREADY_EXISTS` and `DIR_NOT_EMPTY`, from being dropped until the request timeout.
