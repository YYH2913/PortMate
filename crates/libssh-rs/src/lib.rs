...
// Keep the published 0.3.8 source warning-clean under PortMate's newer Clippy.
#![allow(
...
impl Session {
    ...
    /// Try to authenticate using GSSAPI with MIC.
    ///
    /// The username, GSSAPI identities, and credential delegation policy are
    /// configured through [`Session::set_option`] before connecting. The
    /// linked libssh build must include GSSAPI support; builds without it
    /// return [`AuthStatus::Denied`].
    pub fn userauth_gssapi(&self) -> SshResult<AuthStatus> {
        let sess = self.lock_session();
        let res = unsafe { sys::ssh_userauth_gssapi(**sess) };
        sess.auth_result(res, "GSSAPI authentication error")
    }
