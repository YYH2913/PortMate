...
Still pending: real FreeBSD/macOS SSH hosts in the remote-forward integration matrix, terminal compatibility test baselines, broader MCP HTTP client matrix testing, broader transfer/serial integration matrices, cross-platform file-path coverage, a recoverable credential-migration journal, and broader portable-vault recovery/reload workflows. MCP IPC/HTTP tokens remain native-keyring records and are outside profile credential migration.
