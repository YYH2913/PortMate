...
5. A modal settings window opens. `会话 -> 新建会话` and `会话 -> 会话设置` open the session settings dialog; `工具 -> 终端设置` opens the terminal settings dialog.
   ...
   - SSH/TCP/Telnet/Serial reconnect keeps a disconnected runtime in `Reconnecting` and retries in the background until the user closes or manually reconnects the session. SSH/Tmux and TCP/Telnet reload and normalize the latest saved profile before every retry; SSH edits to the endpoint, username, credential references, identities, Jump Hosts, and host-key policy, plus TCP/Telnet host/port or protocol edits, are not frozen at disconnect time. A profile change during an in-flight connection invalidates that result, while disabling reconnect or changing the transport stops the pending worker. TCP/Telnet also reads the latest reconnect flag when an active socket closes, avoiding a stale transition into `Reconnecting`.
