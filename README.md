...
Still pending: real FreeBSD/macOS SSH hosts in the remote-forward integration matrix, terminal compatibility test baselines, broader MCP HTTP client matrix testing, broader transfer/serial integration matrices, cross-platform file-path coverage, bulk migration of SSH/Tmux profile credentials between the native keyring and Stronghold, and broader portable-vault recovery workflows. MCP IPC/HTTP tokens remain native-keyring records and are outside that migration scope.
