...
- Standalone serial analyzer window with capture/delimiter/fixed-length/idle-gap framing, RFC 1055 SLIP decoding, bookmarks, filtering, paging, and exact source-frame export
...
The standalone serial analyzer can frame that capture by source read, Hex delimiter, fixed length, idle gap, or RFC 1055 SLIP. SLIP decoding uses the browser-compatible `slip` library, keeps decoder state across capture reads, and never joins RX with TX. Each derived packet retains its exact source-frame IDs and both decoded payload and original wire bytes; the inspector can switch between them, filtering searches both, and malformed escape sequences remain visible as protocol errors without discarding the decoded packet. Unclosed tails, truncated source reads, bookmarks, paging, and exact source-frame export use the same evidence model in every parser mode.
