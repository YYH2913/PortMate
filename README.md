...
- Real SSH, local Shell PTY, raw TCP, Telnet, serial, SFTP, SCP, and Tmux attach/list/pane inspection/session-window lifecycle/synchronization paths in the Tauri backend
...
The `工具 -> Tmux` dialog reads remote `tmux list-sessions`, `tmux list-windows`, and `tmux list-panes` output through the connected SSH/Tmux runtime. It can attach or create a named tmux session in the active terminal, create and rename windows, rename sessions, confirm and close sessions/windows, and toggle per-window synchronized panes. Mutations accept only fixed backend actions with bounded, shell-quoted targets and names, refresh from the remote snapshot before updating the UI, and record successful lifecycle changes as session system events. Shared SSH helper-command capture rejects stdout above 4 MiB and stderr above 64 KiB before appending the overflowing channel chunk, so abnormal Tmux, tunnel-probe, or Sysmon output cannot grow desktop memory without bound or be silently parsed as a truncated result.
