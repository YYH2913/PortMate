...
5. A modal settings window opens. `会话 -> 新建会话` and `会话 -> 会话设置` open the session settings dialog; `工具 -> 终端设置` opens the terminal settings dialog.
   ...
   - SSH/TCP/Telnet/Serial reconnect keeps a disconnected runtime in `Reconnecting` and retries in the background until the user closes or manually reconnects the session. For SSH/Tmux, every retry reloads and normalizes the latest saved profile; edits to the endpoint, username, credential references, identities, Jump Hosts, and host-key policy are not frozen at disconnect time. A profile change during an in-flight handshake invalidates that result, while disabling reconnect or changing the transport stops the pending worker.
...
Still pending: real FreeBSD/macOS SSH hosts in the remote-forward integration matrix, terminal compatibility test baselines, broader MCP HTTP client matrix testing, broader transfer/serial integration matrices, cross-platform file-path coverage, native keyring/Stronghold fault-injection matrices on Windows/macOS/Linux, and richer manual diagnostics for migration conflicts. MCP IPC/HTTP tokens remain native-keyring records and are outside profile credential migration.
