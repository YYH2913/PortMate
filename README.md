...
Still pending: remote-forward listener probes for non-Linux servers without `ss`/`netstat`, terminal compatibility test baselines, broader MCP HTTP client matrix testing, broader transfer/serial integration matrices, cross-platform file-path coverage, and advanced portable-vault workflows such as master-password rotation and bulk migration of existing native-keyring records.
