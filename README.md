...
- WindTerm-style OneKeys manager for encrypted Account/SSH credentials, session binding, and explicit terminal injection
...
`工具 -> OneKeys` or WindTerm's `Ctrl+T Ctrl+K` chord (`Meta+T Meta+K` on macOS) opens the encrypted credential manager. Account and SSH entries keep the visible label, username, type, and bound session IDs in the store, while passwords and private-key passphrases are written to the automatic/native keyring or an unlocked Portable Stronghold and persist only as generated Secret references. The manager can explicitly preserve, replace, or clear each saved Secret and manually send the username, password, or passphrase to the active connected bound session. Sensitive sends use the per-session outbound lane and create a control event without readable text; explicitly enabled Raw logging still records exact wire bytes under the existing unredacted logging policy. The SSH connection dialog lists only SSH OneKeys bound to that Profile. Selecting one locks its visible username and sends only the OneKey ID to a dedicated backend command; the backend revalidates type and binding, decrypts the password/private-key passphrase, and clears the runtime clone's saved Profile credential references so the two credential sources cannot be silently mixed. The OneKey password participates in password and keyboard-interactive authentication, while its passphrase unlocks the Profile's configured private-key identities. OneKeys remain separate from Quick Commands. Automatic terminal-prompt completion and OneKey-owned public-key identity selection are not implemented yet.
