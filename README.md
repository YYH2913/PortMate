...
The `工具 -> Tmux` dialog reads remote `tmux list-sessions`, `tmux list-windows`, and `tmux list-panes` output through the connected SSH/Tmux runtime. It can attach or create a named tmux session in the active terminal; create, rename, confirm, and close sessions/windows; split, swap, resize, confirm, and close panes; apply the five built-in tmux window layouts; and toggle per-window synchronized panes. Mutations accept only fixed backend actions with bounded, shell-quoted targets and names, refresh from the remote snapshot before updating the UI, and record successful changes as session system events. Pane resizing is additionally bounded to 1-100 cells per request. Shared SSH helper-command capture rejects stdout above 4 MiB and stderr above 64 KiB before appending the overflowing channel chunk, so abnormal Tmux, tunnel-probe, or Sysmon output cannot grow desktop memory without bound or be silently parsed as a truncated result.
