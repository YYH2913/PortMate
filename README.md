...
The frontend persists a v3 recursive workspace tree whose pane leaves are bounded view groups. Each group retains an ordered list of up to 32 session views plus its active view, alongside nested horizontal or vertical splits, split ratios, the active pane/session, and validated tab colors. Existing v1 flat snapshots, v2 single-view trees, and earlier split localStorage keys migrate in place. Restored trees discard stale sessions, collapse empty branches, repair malformed or duplicate node IDs, clamp ratios to 15..85%, and enforce a 16-pane/eight-level boundary. A pane tab strip switches views without changing the surrounding split. `窗口 -> 移动视图到分组` moves only the active view and collapses its source group only when empty; `窗口 -> 合并当前分组` appends the complete source group to a target with stable deduplication and activates the source view. Both operations reject a target that would exceed 32 views. Splitters support pointer dragging, arrow-key adjustment, Home/End limits, and double-click reset; the same ratios survive restart. WindTerm 2.7-compatible workspace keys use `Alt+Arrow` to focus the geometric neighbor, `Alt+-`/`Alt+\` to split below/right, the Shift variants to split above/left, `Alt+X` to close the active pane, and `Alt+Z` to toggle pane zoom; these keys are captured only while an XTerm workspace owns keyboard focus. `工具 -> 终端设置 -> 快捷键` can record one- or two-stroke Ctrl/Alt/Shift/Meta bindings, explicitly disable commands, restore individual or all defaults, and rejects duplicate or prefix-ambiguous bindings before saving. Two-stroke chords may share a prefix, cancel after 1.2 seconds or an invalid suffix, and consume pending strokes so incomplete shortcuts cannot leak into the terminal; v1 single-stroke keymaps migrate to versioned v2 storage. A pane header or `窗口 -> 移到新窗口` detaches the active view from a multi-pane tree into a restricted Tauri `WebviewWindow` that keeps using the same backend session. The source tree changes only after native window creation succeeds; a non-empty source group remains in place, connect/disconnect controls do not alter the main workspace, and the return control restores the view to its original group when that pane still exists. Browser preview uses a same-origin popup and validated `postMessage` payloads for the same development workflow. Zoom hides sibling branches and splitters without unmounting their XTerm instances, and directional focus moves the zoom to the selected pane. The Window menu can also swap the complete active pane with its geometric neighbor. Its horizontal split creates a right-hand view and its vertical split creates a lower view, matching WindTerm's direction semantics. Startup mode can connect no sessions, every view in the last restored groups, or a configured session list after the first profile load; targets are deduplicated and credential-requiring sessions are opened sequentially. `会话 -> 还原布局` reloads and reconciles the saved snapshot against the current profile set.
