...
`Disconnected`, `Blocked`, and `Error` sessions are retried, while `Connecting`, `Connected`, and
`Reconnecting` sessions are left to their existing runtime. This prevents a prior failure from
silently disabling startup recovery without issuing duplicate opens for active sessions.
...
emits approximately 359.8 kB of main JS, 467.5 kB of terminal core JS, 120.4 kB of WebGL JS,
140.4 kB of main CSS, 44.9 kB for the key manager, 38.5 kB for session settings, 13.3 kB for the MCP
workspace, 17.2 kB for the file manager, 12.2 kB for the OneKeys manager, 11.6 kB for terminal
settings, 4.8 kB for session/terminal context menus, 11.3 kB for the Sysmon workspace, 11.2 kB for
the log workspace,
5.0 kB for the tunnel dialog, 4.2 kB for the transfer dialog, 4.0 kB for the SSH credential prompt,
3.7 kB for one-time MCP approval, 2.9 kB for the lazy workspace utility panels, 4.6 kB for the view
context menu, a 0.7 kB notice dialog, and a 0.5 kB browser export helper, with xterm CSS split
alongside them. The 1.8 kB command-history state module, 1.4 kB
terminal-buffer module, 2.2 kB terminal-selection/online-search module, and browser download code
load only when used; the previous approximately
...
health path against OpenSSH 9.6/9.7/9.9, Debian bookworm OpenSSH, Debian trixie OpenSSH, Ubuntu 24.04 OpenSSH, and Dropbear. It also kills each
...
The same seven-server matrix runs X/Y/ZModem activity on independent SSH runtimes, including Alpine
lrzsz builds and Debian, Debian trixie, and Ubuntu YModem fallback, and verifies disconnect failure, partial progress, final-file
...
`npm run test:ssh-gssapi-compat` provisions an Ubuntu 24.04 OpenSSH server and MIT Kerberos realm,
then verifies successful `gssapi-with-mic`, strict host-key rejection, a missing client ticket, and
server-disabled GSSAPI. The success case also covers TOFU persistence, PTY shell I/O, resize,
keepalive/exec health, and bounded runtime cleanup.
...
only `gssapi-with-mic`, Linux uses the system libssh backend for PortMate host-key verification,
authentication, PTY/shell setup, interactive I/O and resize, exec-based Tmux/Sysmon work, and
keepalive/exec health probes. A russh test server to libssh client regression covers normalized
stdout, stderr, exit status, and EOF handling. Mixed authentication still reports that russh 0.62.4
has no client GSSAPI exchange. The libssh path also rejects Proxy, Jump Host, and agent forwarding;
SFTP/SCP and tunnels remain explicit russh compatibility paths.
...
channel tests are complemented by a real Ubuntu 24.04 OpenSSH and MIT Kerberos matrix covering
successful authentication, strict host-key rejection, a missing ticket, and server-disabled GSSAPI.
Additional Kerberos and SSH server implementations and mixed authentication remain unverified.
...
Still pending: additional Kerberos/GSSAPI server implementations and unsupported libssh transport
combinations, including mixed authentication, Proxy/Jump Host, agent forwarding, SFTP/SCP, and tunnels,
