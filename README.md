...
3. Use the four top-menu groups:
   ...
   - `会话`: create, edit, connect, disconnect, and duplicate a session
   - `终端`: search, go to line, selection/key modes, synchronized/free input, and text export
   - `工作区`: toggle resource/file/history/sender dock tabs, Quick Bar, status bar, and layout restore
   ...
   together. Panels in the same dock share a compact tab strip; drag a tab to an edge target to move
   it between docks. Drag a dock boundary to resize it, use the separator arrow keys for precise
