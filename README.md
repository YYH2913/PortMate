...
5. A modal settings window opens. `会话 -> 新建会话` and `会话 -> 会话设置` open the session settings dialog; `工具 -> 终端设置` opens the terminal settings dialog.
   ...
   - SSH/TCP/Telnet/Serial reconnect keeps a disconnected runtime in `Reconnecting` and retries in the background until the user closes or manually reconnects the session. SSH/Tmux, TCP/Telnet, and Serial reload and normalize the latest saved profile before every retry; SSH endpoint/authentication edits, TCP/Telnet host/port or protocol edits, and Serial port/line-setting edits are not frozen at disconnect time. A connection-setting change during an in-flight attempt invalidates that result, while disabling reconnect or changing the transport stops the pending worker. TCP/Telnet and Serial also read the latest reconnect flag when an active transport closes, avoiding a stale transition into `Reconnecting`. TCP/Telnet profiles persist a 100-60,000 ms reconnect delay plus an OS keepalive toggle, idle time, probe interval, and retry count; defaults are 1,000 ms and 30/10/3. Supported kernels apply those values without injecting protocol bytes, and legacy profiles receive the defaults automatically.
