...
or unknown response IDs, enforces the configured inbound packet-length limit before allocating the
declared payload, and routes valid out-of-order responses by request ID. The matrix kills each of the
nine OpenSSH/Dropbear servers
...
the expected failure stage, and every health case has a hard test deadline. Thirty-six transfer fault
...
status `99`, a malformed response packet type, a truncated response, an oversized declared packet,
and an unknown response request ID. Every injected status or invalid response must fail within five
